...
class SAC(object):
    ...
    def save_model(self, env_name, suffix=""):
        if not os.path.exists('models/{}/'.format(env_name)):
            os.makedirs('models/{}/'.format(env_name))
        actor_path = "models/{}/sac_model_{}_{}.pt".format(env_name, env_name, suffix)
        print('Saving models to {} '.format(actor_path))
        torch.save([self.policy.state_dict(), self.critic.state_dict()], actor_path)
    # Load model parameters
    def load_model(self, model_path):
        print("Loading models form {}".format(model_path))
        policy_dict, critic_dict = torch.load(model_path)
